    ...
    return (
        <div className="w-full">
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    onClick={() => router.push("/manager/chats")}
                ...
                <div className="col-span-1 space-y-4">
                    <div className="bg-white rounded-lg shadow p-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                <label className="text-xs font-semibold text-slate-500 uppercase">Đã trích xuất</label>
                                <div className="mt-1">
                                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                                        chat.isExtracted ? "bg-blue-100 text-blue-800" : "bg-yellow-100 text-yellow-800"
                                    }`}>
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-sm text-slate-700">{formatDate(chat.createdAt)}</p>
                            </div>
                            <div>
                                <label className="text-xs font-semibold text-slate-500 uppercase">Cập nhật lần cuối</label>
                                <p className="mt-1 text-sm text-slate-700">{formatDate(chat.updatedAt)}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {chat.isExtracted && !chat.isSpam && chat.extractedData && (
                        <div className="bg-white rounded-lg shadow p-4">
                            <h2 className="text-lg font-semibold text-slate-800 mb-4">Dữ liệu trích xuất</h2>
                            <div className="space-y-2 text-sm">
                                {chat.extractedData.username && (
                                    <div>
                                        <label className="font-semibold text-slate-600">Tên người dùng:</label>
                                        <p className="text-slate-700">{chat.extractedData.username}</p>
                                    </div>
                                )}
                                {chat.extractedData.phone && (
                                    <div>
                                        <label className="font-semibold text-slate-600">Điện thoại:</label>
                                        <p className="text-slate-700">{chat.extractedData.phone}</p>
                                    </div>
                                )}
                                {chat.extractedData.interest && (
                                    <div>
                                        <label className="font-semibold text-slate-600">Sở thích:</label>
                                        <p className="text-slate-700">{chat.extractedData.interest}</p>
                                    </div>
                                )}
                                {chat.extractedData.category && (
                                    <div>
                                        <label className="font-semibold text-slate-600">Danh mục:</label>
                ...
                <div className="col-span-2 bg-white rounded-lg shadow p-4">
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                        {chat.messages && chat.messages.length > 0 ? (
                            ...
                            <div className="text-center py-8 text-slate-500">Không có tin nhắn</div>
                        )}
                    </div>
                </div>
            </div>
