...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        try {
            const success = await authService.login(account, password)
            if (success) {
                router.push("/")
            } else {
            ...
            <section className="w-full max-w-md bg-white rounded-lg shadow p-8">
                ...
                <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="mt-4 h-12 w-full bg-blue-600 text-white rounded-md text-lg font-medium hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                    >
