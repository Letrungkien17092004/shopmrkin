...
export type VariantConstructorParam = {
    id: string,
    name: string,
    sku: string,
    price: number,
...
export default class ProductService {
    ...
    async createData(options: Pick<Product, "productName" | "description" | "category" | "imgUrl" | "variants">): Promise<Product> {
        try {
    ...
    async findById(id: string): Promise<Product | null> {
        return demoProductData.find(prod => prod.id === id) ?? null
    }
}